  ...
  public Connessione(IDbOperationSQL dbOp, String sDsn) {
    ...
    try {
      ...
      if (rst.next()) {
        ...
        if(istanza.length() != 0) {
          query = "SELECT DATABASE_DRIVER, DATABASE_LINK, USER_ORACLE, PASSWORD_ORACLE"+
                  "  FROM AD4_ISTANZE"+
          ...
          rst = dbOp.getRstSet();
          if (rst.next()) {
            driver      = rst.getString("DATABASE_DRIVER");
            if (driver == null) {
              driver      = "oracle.jdbc.driver.OracleDriver";
            }
            connessione = rst.getString("DATABASE_LINK");
            if (connessione != null) {
              if (connessione.indexOf("@") == -1) {
                connessione = "jdbc:oracle:thin:@"+connessione;
              }
            }
