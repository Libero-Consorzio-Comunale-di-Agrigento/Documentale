CREATE OR REPLACE FUNCTION F_PATH_SOTTOALBERO_ML(A_FOLDER VARCHAR2,A_FOLDER_STOP VARCHAR2,
                                              A_COLLEGAMENTO NUMBER,A_TYPE VARCHAR2,A_LINGUA VARCHAR2)
RETURN VARCHAR2 IS
/*****************************************************************************************
...
BEGIN
  BEGIN
    FOR CTREE IN C_TREE LOOP
        ...
        BEGIN
          SELECT GDM_UTILITY.F_MULTILINGUA (DECODE(C1.NOME,'WRKSPUTENTE','Utente','WRKSPSISTEMA','Sistema',C1.NOME),A_LINGUA ) AS NOMEPARENT,
                 GDM_UTILITY.F_MULTILINGUA (C2.NOME,A_LINGUA ) AS NOMECHILD,
    ...
    IF DESCR IS NULL THEN
       BEGIN
         SELECT GDM_UTILITY.F_MULTILINGUA (DECODE(NOME,'WRKSPUTENTE','Utente','WRKSPSISTEMA','Sistema',NOME),A_LINGUA )
           INTO DESCR
           FROM CARTELLE
